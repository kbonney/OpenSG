...
###############     OpenSG         ############################
########### Tapered Timo Dehomogenized Buckling (Solid Elements) With plots ###################
...
from ufl import lt,SpatialCoordinate, as_tensor,  Measure, grad
...
EIGVAL,EIGVEC_R,EIGVEC_I=[],[],[]
V0_all,V1s_all=[],[]
leftface_all,rightface_all= [],[]
V_all, VV_all, dx_all=[], [], []
x_all=[]
...
## Extract reaction force data from beamdyn.out file ##

data=np.loadtxt('bd_bar_urc.out', delimiter=',', skiprows=0, dtype=str)
index=data[1].split()
p=3
...
for seg in range(n_seg):
    ...
    for f in range(6):
        ...
        if f>2:
            sc=pp+n_seg*(3+f)+seg
        beam_reac.append([index[sc],float(last_data[sc])])
     #   print(f,index[sc])
    beam_f.append(beam_reac)

#beam_f['segment_id][force_id][force_value]
...
for segment in seg_range:
 ...
 #   V0_all.append(V0)
  #  V1s_all.append(V1s)
  #  leftface_all.append(entity_mapl)
  #  rightface_all.append(entity_mapr)
  #  V_all.append(V)
    VV=dolfinx.fem.functionspace(mesh, basix.ufl.element(
  ...
  #  Deff_srt=taper_timo[seg_id]
    ...
    rf=[beam_f[int(segment)][k][1] for k in range(6)]

    print(' Eigen buckling analysis: Segment-', int(segment)+1)
 #   print('\n BeamDyn Reaction Force \n')
    for i in range(6): print(beam_f[int(segment+1)][i][0])
    
    FF=np.array((rf[2],rf[0],rf[1],rf[5],rf[3],rf[4])) # BeamDyn --> VABS convention 
    conv=['F1','F2','F3', 'M1','M2','M3']
    print('\n VABS reaction force (1- beam axis): \n')
    for k in range(6):
        print(conv[k], FF[k])
 ...
 #   VV=dolfinx.fem.functionspace(mesh, basix.ufl.element(
  ...
  #  left_facets = mesh.locate_entities_boundary(mesh, fdim, left)  --> entity_mapl
   # right_facets = mesh.locate_entities_boundary(domain, fdim, right) --> entity_mapr
    
    u_L = np.array([0, 0, 0], dtype=dolfinx.default_scalar_type)
    bcs = [
        dirichletbc(u_L, locate_dofs_topological(V, fdim, entity_mapl), V),
        dirichletbc(u_L, locate_dofs_topological(V, fdim, entity_mapr), V)]
 #   bcs = [
 #   dirichletbc(u_L, locate_dofs_topological(V, fdim, entity_mapl), V),
 #   dirichletbc(
...
#]
    ...
    kgform = -sum([inner(sigma_prestress(i)[0],grad(du).T*grad(u_))*dx(i) for i in range(nphases)])
 ...
 #   EIGVEC_R.append(eigvec_r[0])
 #   EIGVEC_I.append(eigvec_i[0])
    
    print('Eigenvalue solved for segment',int(segment+1),'\n' )
    print('Critical Eigen value:', eigval[0:5])
...
#    for i in range(1):
    ...
    i=0 
    pl = pyvista.Plotter(shape=(1, 1))
    eigenmode = f"Segment_{i:02}"
    pl.add_text(
        f"Eigenmode {int(segment)+1}",
        font_size=12,
    )
    ...
    pl.add_mesh(u_grid.warp_by_vector(eigenmode, factor=20), show_edges=True)
    pl.view_isometric()
...
# 3D Local Stress Plots
V2 = dolfinx.fem.functionspace(mesh, basix.ufl.element(
        "DG", mesh.topology.cell_name(), 0, shape=(6,6))) 
stiffness=Function(V2)
for i,sub in enumerate(subdomains.values):
    stiffness.vector[36*i:36*i+36]=CC[sub].flatten() 


u_stress=Function(VV)
fexpr1=dolfinx.fem.Expression(stiffness*strain_3D,VV.element.interpolation_points(), comm=MPI.COMM_WORLD)
u_stress.interpolate(fexpr1) 
...
for i in range(n_components):
    ...
    # Attach the first component to the grid
    grid.point_data["Stress_"+str(i+1)] = stress_xx_values.real  # Nodal values for σ_xx
    grid.set_active_scalars("Stress_"+str(i+1))
...
# 3D Local Strain Plots

n_components=6
for i in range(n_components):
    stress_xx_values = strain_3D.x.array[i::n_components]  # Extract every 6th value (σ_xx) 
    pyvista.start_xvfb()
    u_topology, u_cell_types, u_geometry=dolfinx.plot.vtk_mesh(VV)
    grid = pyvista.UnstructuredGrid(u_topology, u_cell_types, u_geometry)
